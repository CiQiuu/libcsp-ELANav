...
# ── Marcadores que imprime el binario ─────────────────────────────────────────
MARK_OK   = "Guardado en gs_received_video.mp4"
MARK_FAIL = "ERROR SFP"
MARK_SIZE = "Video recibido:"            # "Video recibido: 2684090 bytes"
MARK_TIME = "Tiempo de transferencia"    # "Tiempo de transferencia SFP: 13650.21 s (227.50 min)"
...
def drain_until(markers_ok, markers_fail, deadline):
    """
...
def main():
    ...
    ap.add_argument("--md5",     default=None, help="MD5 esperado del video original")
    args = ap.parse_args()
    ...
    # ── Lanzar el GS ────────────────────────────────────────────────────────
    print("=" * 64)
    print(f"  PRUEBA DE ESTRÉS DE VIDEO RF — ELANav")
    ...
    with open(csv_path, "w", newline="") as cf:
        ...
        for cyc in range(1, args.cycles + 1):
            ...
            # Borrar video anterior para no contar un MD5 viejo como éxito
            try:
            ...
            print(f"   → comando 'v' enviado, esperando transferencia…")

            # Esperar resultado
            deadline = t_ini + args.timeout
            info = drain_until([MARK_OK], [MARK_FAIL], deadline)
            ...
            md5_match = ""
            if info["result"] == "OK":
                if args.md5 and os.path.exists(VIDEO_OUT):
                    got = md5_of(VIDEO_OUT)
